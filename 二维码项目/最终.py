...
class Application(tk.Frame):
    ...
    def video_loop(self):
        ...
        while self.win_flag:
            ...
            if data and self.win_flag :
                ...
                messagebox.showinfo("提示"," 检测到二维码，已填充到字符串输入栏")
                if cv2.waitKey(0) == 27:
                    # self.cap.release()
    ...
    def int_window(self):
        ...
        self.selector = ttk.Combobox(frame1,values=("一元函数","二元函数"),width="15")
        self.selector.grid(row=0,column=1)
        self.selector.current(0)
        self.selector.bind("<<ComboboxSelected>>", self.selector_listener)

        #Button
        button=tk.Button(frame1,text="函数图像",width=15,command=self.Buttonclick)
        button.grid(row=0,column=3,padx=5)
        button1=tk.Button(frame1,text="生成二维码",width=10,command=self.Button1click)
